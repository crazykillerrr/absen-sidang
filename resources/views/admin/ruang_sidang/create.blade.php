...
<div class="mb-4">
    ...
    <h4 class="fw-bold" style="color: var(--text-primary);">Tambah Ruang Sidang</h4>
</div>
...
<div class="row">
    <div class="col-lg-6 col-12">
        <div class="card border-0 shadow-sm rounded-4 p-4 p-md-5" style="background-color: var(--bg-secondary);">
            ...
            <form method="POST" action="{{ route('admin.ruang-sidang.store') }}">
                @csrf

                <!-- Nama Ruang -->
                <div class="mb-3">
                    <label for="nama_ruang" class="form-label fw-semibold">Nama Ruangan</label>
                    <input type="text" name="nama_ruang" id="nama_ruang" class="form-control form-control-custom @error('nama_ruang') is-invalid @enderror" placeholder="Contoh: Ruang Sidang Cakra" value="{{ old('nama_ruang') }}" required>
                    @error('nama_ruang')
                ...
                <div class="mb-4">
                    ...
                    <select name="jenis_ruang" id="jenis_ruang" class="form-select form-control-custom @error('jenis_ruang') is-invalid @enderror" required>
                        ...
                        @foreach ($jenisRuangs as $jenis)
                            <option value="{{ $jenis }}" {{ old('jenis_ruang') === $jenis ? 'selected' : '' }}>{{ $jenis }}</option>
                        @endforeach
                    </select>
                    @error('jenis_ruang')
                        <div class="invalid-feedback">{{ $message }}</div>
