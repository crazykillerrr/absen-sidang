            ...
            <form method="POST" action="{{ route('admin.perkara.update', $perkara->id) }}">
                ...
                <div class="row">
                    ...
                    <div class="col-md-8 col-12 mb-3">
                        ...
                        <input type="text" name="nomor_perkara" id="nomor_perkara" class="form-control form-control-custom @error('nomor_perkara') is-invalid @enderror" placeholder="Contoh: 120/G/2026/PTUN.JKT" value="{{ old('nomor_perkara', $perkara->nomor_perkara) }}" required>
                        @error('nomor_perkara')
                ...
                <div class="mb-3">
                    ...
                    <select name="ketua_majelis" id="ketua_majelis" class="form-select form-control-custom @error('ketua_majelis') is-invalid @enderror" required>
                        <option value="" disabled>-- Pilih Ketua Majelis --</option>
                        @foreach ($hakims as $hakim)
                            <option value="{{ $hakim->id }}" {{ old('ketua_majelis', $ketuaMajelisId) == $hakim->id ? 'selected' : '' }}>{{ $hakim->nama }}</option>
                        @endforeach
                    </select>
                    @error('ketua_majelis')
                        <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="col-md-6 col-12">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="hakim_anggota[]" value="{{ $hakim->id }}" id="hakim_anggota_{{ $hakim->id }}" 
                                        {{ (is_array(old('hakim_anggota')) && in_array($hakim->id, old('hakim_anggota'))) || (!is_array(old('hakim_anggota')) && in_array($hakim->id, $hakimAnggotaIds)) ? 'checked' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    <select name="panitera_pengganti" id="panitera_pengganti" class="form-select form-control-custom @error('panitera_pengganti') is-invalid @enderror" required>
                        <option value="" disabled>-- Pilih Panitera Pengganti --</option>
                        @foreach ($pps as $pp)
                            <option value="{{ $pp->id }}" {{ old('panitera_pengganti', $currentPpId) == $pp->id ? 'selected' : '' }}>{{ $pp->nama }}</option>
                        @endforeach
                    </select>
